...
const petvaccine = async (req, res) => {
  ...
  const petdeworming = async (req, res) => {
    try {
      const  {id,type,method,deworming_date,deworming_reminder,deworming_price,used_product,user} = req.body
      if (!req?.files?.picture)
        return res.status(400).json({success: false, message: "Please upload the ad image image."});
      const file = req.files.picture;
      const result = await cloudinary.uploader.upload(file.tempFilePath, {
  ...
  const petdisease = async (req, res) => {
    try {
      const  {id,name,title,description,diagnosis_date,start_date,end_date,reminder_date,diagnosis,price,user} = req.body
        const data = await Medical.create({
  ...
  const petsurgery = async (req, res) => {
    try {
      const  {id,type,date,description,name,reminder_date,price,user} = req.body
        const data = await Medical.create({
  ...
  const petdose = async (req, res) => {
    try {
      const  {id,name,dose,frequency,reminder_date,start_date,end_date,price,user} = req.body
        const data = await Medical.create({
          "drug_name": name,
              "dosage": dose,
              "frequency": frequency,
  ...
  const petactivity = async (req, res) => {
    try {
      const  {id,type,description,date,duration,travelled,altitude,location,difficult,fun,user} = req.body
