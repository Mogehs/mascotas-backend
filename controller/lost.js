...
if (!req?.files?.picture)
  ...
  const result = await cloudinary.uploader.upload(file.tempFilePath, {
   ...
   const data = await Lost.create({
    ...
    const users = await Lost.find().populate('user', 'device_token firstname lastname phone');


   const petsToNotify = users.filter(pet => 
    pet.user && 
    ...
    const notificationPromises = users.map(async (pet) => {
      if (pet.user?.device_token) {
        ...
        return sendPushNotification(pet.user.device_token,notification,name,result.secure_url,location,time,contact,date,details);
      }
      return null;
    });
...
async function sendPushNotification(deviceToken,notification,name,image,location,time,contact,date,details) {
